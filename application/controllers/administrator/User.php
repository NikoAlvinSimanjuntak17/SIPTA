		...
		if ($this->form_validation->run()) {
			...
			if ($save_user) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->response['message'] = 'Data telah berhasil disimpan. '.anchor('administrator/user/edit/' . $save_user, 'Edit Permission').' or  '.anchor('administrator/user', ' Go back to list');
				} else {
					set_message('Data telah berhasil disimpan. '.anchor('administrator/user/edit/' . $save_user, 'Edit User'));
	        		$this->response['success'] = true;
	...
	public function edit_profile_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($save_user) {
				...
				$this->data['id'] 	   = $id;
				$this->data['message'] = 'Data telah berhasil diperbarui. '.anchor('administrator/user', ' Go back to list');
			} else {
