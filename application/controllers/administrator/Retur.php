...
class Retur extends Admin	
{
	...
	public function add_save()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($save_retur) {
				...
				$this->db->update('barang', ['status' => 'tersedia'], ['nama_barang' => $nama_barang]);

				if ($this->input->post('save_type') == 'stay') {
		
					$this->data['success'] = true;
					$this->data['id'] 	   = $save_retur;
					$this->data['message'] = 'Data telah berhasil disimpan. '.anchor('administrator/retur/edit/' . $save_retur, 'Edit Retur').' or  '.anchor('administrator/retur', ' Go back to list');
				} else {
					set_message('Data telah berhasil disimpan. '.anchor('administrator/retur/edit/' . $save_retur, 'Edit Retur'), 'success');
		...
		if ($this->form_validation->run()) {
			...
			if ($save_retur) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $id;
					$this->data['message'] = 'Data telah berhasil diperbarui. '.anchor('administrator/retur', ' Go back to list');
				} else {
	...
	*/
	public function delete($id)
	{
		...
		if ($remove) {
            set_message('Retur has been deleted.', 'success');
		} else {
            set_message('Error delete retur.', 'error');
		}

		redirect('administrator/retur');
	}
	...
	*/
	public function get_berkas_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => 'id_retur',
            'upload_path'       => 'uploads/retur/',
            'delete_endpoint'   => 'administrator/retur/delete_berkas_file'
        ]);
	}
